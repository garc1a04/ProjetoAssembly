.data
	diretorio: .asciiz "EntradaDados.txt"
	salvarArq: .asciiz "SaidaDados.txt"
...
.text
	...
	move	$t0,$zero	#nosso i
	move	$t6,$zero	#nosso j
	
	add 	$t7, $zero, $s2 
	ordenar:
		beq 	$t0,$t7,seguirEmFrente		# if(indexAtual == tamanhoVetor);
		
		move	$t2,$zero			# auxiliar do i
		add	$t6,$t2,4			# j = i+1
		loop:
			beq	$t6,$t7,continuar2	# if(j == tamanhoVetor);
			
			lw 	$t1, vetorInteiro($t2)  # pegar o valor do vetor(i) e coloca no t1
			lw	$t3, vetorInteiro($t6)	# pegar o valor do vetor(i+1) e coloca no t3
			
			bgt 	$t1,$t3,troca  		# if(vetor(i) > vetor(i+1));
			
			continuar:
				add 	$t2,$t2,4	# Auxiliar do i+1 na index
				add	$t6,$t6,4	# j+1
			j	loop
		
		continuar2:
			add	$t0,$t0,4		# i+=tamanho	
			j	ordenar
	...
	#passar para STRING
	move	$s1,$s2			#S1 armazena o tamanho da entrada do arquivo
	li	$s2,1024 	
